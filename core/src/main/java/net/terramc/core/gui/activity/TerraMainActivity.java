...
public class TerraMainActivity extends TabbedActivity {
  ...
  public TerraMainActivity(TerraAddon addon) {
    this.addon = addon;
    this.register("terra_overview", new DefaultComponentTab(Component.translatable("terramc.ui.activity.overview"), new TerraOverviewActivity()));
  ...
  public void updateStaffActivity() {
    ...
    String cloudId = "terra_cloud_main";
    ...
    if(this.addon.rankUtil().canControlCloud()) {
      if(this.getElementById(cloudId) == null) {
        ...
        //this.register(cloudId, new DefaultComponentTab(Component.translatable("terramc.ui.activity.cloud.navigation"), this.addon.terraCloudActivity));
      }
    } else {
