...
import net.terramc.core.data.StaffSettings;
import net.terramc.core.group.StaffGroup;
...
public class ApiUtil {
  ...
  public void loadRankData(UUID uuid) {
    ...
    URLResolver.readJson(baseUrl + "staff?uuid="+uuid+"&source=Addon_LM4", true, new WebResponse<JsonElement>() {
      @Override
      public void success(JsonElement result) {
  ...
  public boolean loadPlayerStats(UUID uuid) {
    URLResolver.readJson(baseUrl + "stats?uuid="+uuid+"&source=Addon_LM4", true, new WebResponse<JsonElement>() {
      @Override
      public void success(JsonElement result) {
